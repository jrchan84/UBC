...
void produce() {
  ...
  while(items >= MAX_ITEMS) {
    uthread_cond_wait(isEmpty);
  }
  items += 1;
  assert (items >= 0 && items <= MAX_ITEMS);
  histogram [items] ++;
  uthread_cond_broadcast(hasItems);
...
void consume() {
  ...
  assert (items >= 0 && items <= MAX_ITEMS);
  histogram [items] ++;
  uthread_cond_broadcast(isEmpty);
  uthread_mutex_unlock(mx);
}
...
int main (int argc, char** argv) {
  ...
  hasItems = uthread_cond_create(mx);
  isEmpty = uthread_cond_create(mx);

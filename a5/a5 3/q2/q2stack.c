...
char* pop() {
  ...
  char* aName = malloc(sizeof(e->name));  // Before freeing e, allocate new memory to make a copy of e->name
  strncpy(aName, e->name, sizeof(e->name)); // copy name.

  top = e->next;
  free (e);
  return aName; // return copy of e->name
}

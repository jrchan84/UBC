...
template <typename T>
void BinaryTree<T>::printPaths(const Node* subRoot, int paths[], int pathsLength) const
{
  if (subRoot == NULL) {
    return;
  }

  paths[pathsLength] = subRoot->elem;
  pathsLength++;
  ...
  if (subRoot->left == NULL && subRoot->right == NULL) {
    ...
    printPaths(subRoot->left, paths, pathsLength);
    printPaths(subRoot->right, paths, pathsLength);
  }
}
...
int BinaryTree<T>:: sumDistances(const Node* subRoot, int count) const {
    ...
    if (subRoot->left == NULL && subRoot->right == NULL) {
      ...
      return count + sumDistances(subRoot->right, temp);
    } else if (subRoot->right == NULL) {
      return count + sumDistances(subRoot->left, temp);
    } else {
      return count + sumDistances(subRoot->left, temp) + sumDistances(subRoot->right, temp);
    }
}
